...
export class SleepController {
    ...
    //             .getAllByChild(req.params.user_id, new Query().deserialize(req.query))
    ...
    //             .getByIdAndChild(req.params.sleep_id, req.params.user_id, new Query().deserialize(req.query))
    ...
    //         const result = await this._sleepService.updateByChild(sleep, req.params.user_id)
    //         if (!result) return res.status(HttpStatus.NOT_FOUND).send(this.getMessageNotFoundSleep())
    //         return res.status(HttpStatus.OK).send(result)
    //     } catch (err) {
    //         const handlerError = ApiExceptionManager.build(err)
    ...
    public async removeSleepByUser(@request() req: Request, @response() res: Response): Promise<Response> {
        try {
            const result: boolean = await this._sleepService.removeByChild(req.params.sleep_id, req.params.user_id)
            if (!result) return res.status(HttpStatus.NOT_FOUND).send(this.getMessageNotFoundSleep())
