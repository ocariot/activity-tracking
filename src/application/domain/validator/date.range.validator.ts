...
export class DateRangeValidator {
    public static validate(startDate: string, endDate: string): void | ValidationException {
        ...
        const message: string = Strings.ERROR_MESSAGE.DATE_RANGE_INVALID
            .replace('{0}', startDate)
            .replace('{1}', endDate)

        if (diffDays < 0) {
            throw new ValidationException(message, Strings.ERROR_MESSAGE.DATE_RANGE_INVALID_DESC)
        }

        if (diffDays > DateRangeValidator.getLimitDays(startDate, endDate)) {
            throw new ValidationException(message, Strings.ERROR_MESSAGE.DATE_RANGE_EXCEED_YEAR_DESC)
        }
    }
